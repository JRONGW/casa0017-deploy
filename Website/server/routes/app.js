...
@@ -43,98 +30,52 @@ app.get("/api/indicators", async (req, res, next) => {
...
app.get("/api/country/:iso3/panel", async (req, res, next) => {
  const envMap = {
    co2: "EN.ATM.CO2E.PC",
@@ -160,12 +101,10 @@ app.get("/api/country/:iso3/panel", async (req, res, next) => {
...
@@ -180,21 +119,43 @@ app.get("/api/country/:iso3/gdp", async (req, res, next) => {
      [req.params.iso3]
    );
    res.json(rows);
  } catch (err) { next(err); }
